...
class Minesweeper:
    """
    Minesweeper game representation mahshid
    """

    def __init__(self, height=8, width=8, mines=8):

        # Set initial width, height, and number of mines mahshid
        self.height = height
        self.width = width
        self.mines = set()

        # Initialize an empty field with no mines mahshid
        self.board = []
        ...
        # At first, player has found no mines mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
        self.mines_found = set()

    def print(self):
        """
        Prints a text-based representation
        of where mines are located.mahshid moolaei
        """
        for i in range(self.height):
            print("--" * self.width + "-")
            for j in range(self.width):
    ...
    def nearby_mines(self, cell):
        ...
        # Keep count of nearby minessssssssssssssssssssssssssssssss
        count = 0

        # Loop over all cells within one row and columnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):

                # Ignore the cell itselffffffffffffffffffffffffffffffffffffffffhhhhhhhhhhhhhhhhhhhhh
                if (i, j) == cell:
                    continue

                # Update count if cell in bounds and is mine mahshid
