...
#include "math.h"
#include "string.h"
#define MP(pixel) ((pixel) > 255 ? 255 : (pixel))
...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            *imgPtr = (RGBTRIPLE){.rgbtBlue = meanValue, .rgbtGreen = meanValue, .rgbtRed = meanValue};
        }
    }
}
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            image[i][j] = (RGBTRIPLE){.rgbtBlue = round(sB / base), .rgbtGreen = round(sG / base), .rgbtRed = round(sR / base)};
        }
    }
}
...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    RGBTRIPLE tmpImg[height][width];
    memcpy(tmpImg, image, height * width * sizeof(RGBTRIPLE));
    const int GxWeights[3][3] = {{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
    const int GyWeights[3][3] = {{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            image[i][j] = (RGBTRIPLE){.rgbtBlue = MP(sqrtOfa2Plusb2(GxB, GyB)),
